...
simple_metalarray = MtlArray(ones(Float32, 1,1))
rp_metalarray = MtlArray(ones(Float32, 1,1,1))
...
@testset "General constructors" begin #reasonable basic tests to ensure that multiple dispatch is correctly working to construct the correct types of Calculators
    ...
    @test NQCCalculators.Create_Cache(model, 1, simple_metalarray) isa NQCCalculators.QuantumModel_Cache
    @test NQCCalculators.Create_Cache(model, 1, 2, simple_metalarray) isa NQCCalculators.RingPolymer_QuantumModel_Cache

    model = NQCModels.Free()
    @test NQCCalculators.Create_Cache(model, 1, simple_metalarray) isa NQCCalculators.ClassicalModel_Cache
    @test NQCCalculators.Create_Cache(model, 1, 2, simple_metalarray) isa NQCCalculators.RingPolymer_ClassicalModel_Cache
    ...
    @test NQCCalculators.Create_Cache(model, 1, simple_metalarray) isa NQCCalculators.ClassicalFrictionModel_Cache
    @test NQCCalculators.Create_Cache(model, 1, 2, rp_metalarray) isa NQCCalculators.RingPolymer_ClassicalFrictionModel_Cache
end
...
@testset "RingPolymer_ClassicalModel_Cache" begin
    ...
    for I in NQCCalculators.beads(cache)
        @test NQCCalculators.get_potential(cache, r)[I] !== hcat(NQCModels.potential(model, r[:,:,I]))
    ...
    # Check all the results
    for I in NQCCalculators.beads(cache)
        @test cache.potential[I] ≈ hcat(NQCModels.potential(model, r[:,:,I]))
        @test cache.derivative[:,:,I] ≈ NQCModels.derivative(model, r[:,:,I])
    end
    @test cache.centroid ≈ centroid
    @test cache.centroid_potential ≈ hcat(NQCModels.potential(model, centroid))
